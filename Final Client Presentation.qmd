---
title: "CPP Farm Store Omnichannel Strategy"
subtitle: "Final Client Presentation"
author: "Kevin, Alexa, Sandy Zhu, Brian, Ron Sabilla"
date: today
format:
  revealjs:
    theme: simple
    css: cpp-theme.css
    slide-number: true
    transition: slide
    embed-resources: true
---

## Executive Summary

CPP Farm Store is a student-run store at Cal Poly Pomona that sells fresh produce, estate wine, plants, honey, and custom gift items — all grown on site. Most people don't know it exists, and those who do find the website confusing.

**Recommendation:** Build a connected omnichannel strategy by improving Shopify, launching a gift box campaign, setting up Google Merchant Center, and adding loyalty and email tools.

**What we found:** Customers drop off between Awareness and Consideration — not enough product info online, unclear checkout, and zero post-purchase communication.

**Projected results:** 40% increase in gift box orders, 25% jump in traffic, 3% conversion rate, and a growing base of repeat customers.

## Client and Situation Analysis

**About CPP Farm Store**

- Founded on W.K. Kellogg's 1925 horse ranch, now part of Cal Poly Pomona
- Operated by the Huntley College of Agriculture as a student-run enterprise
- Students grow, harvest, market, and sell everything themselves
- Located at 4102 S. University Drive | Open Daily 10AM–6PM

**Products:** Fresh produce, estate wine, plants, honey, eggs, student-made ice cream, and custom gift boxes

## Target Customers

- **CPP Students** — on campus daily but largely unaware the store exists
- **Faculty & Staff** — reliable shoppers, easiest to retain
- **Local Community** — find via Google, blocked by navigation and parking friction
- **Alumni** — emotionally connected, ideal gift box buyers, rely on digital
- **Online-First Shoppers** — expect smooth browsing, checkout, and pickup

## Product Categories

- **Fresh produce & agricultural goods** — fruits, vegetables, eggs, honey
- **Specialty products** — estate wine, student-made ice cream, jams
- **Nursery & ornamentals** — plants and seasonal items
- **Gift products** — customizable gift boxes and fruit packs *(highest margin, lowest visibility)*

## Strengths & Weaknesses

**Strengths**

- Authentic student-run story no competitor can replicate
- High-quality standout products drive word of mouth
- Built-in campus audience every day
- Shopify store already live

**Weaknesses**

- Low awareness on and off campus
- Incomplete Google profile, no retail media presence
- Confusing checkout, no order confirmation or pickup updates
- No loyalty program or post-purchase follow-up

## Opportunities & Challenges

**Opportunities**

- Growing demand for local, mission-driven food
- Seasonal gifting: Mother's Day, graduation, holidays
- Google Shopping tools accessible for small businesses
- Campus event traffic that can convert to online customers

**Challenges**

- Farmers' markets and national gift brands (Amazon, Edible Arrangements) competing for the same budget
- Rotating student staff affects consistency
- Campus parking and navigation deters off-campus visitors

## SWOT Summary

| | Helpful | Harmful |
|---|---|---|
| **Internal** | Campus-grown story, quality products, Shopify foundation, loyal alumni | Low awareness, poor discoverability, no post-purchase communication |
| **External** | Local food trends, Google Shopping, seasonal gifting windows | Stronger competitors, campus barriers, limited budget |


# 3. Key Customer Journey Insight

## 3. Key Customer Journey Insight

### Target Customers

:::: {.columns}

::: {.column width="33%"}

### 🎓 Students

**Convenient & Digital**

On-campus shoppers who value affordability, local products, and digital promotions.

:::

::: {.column width="33%"}

### 👩‍💼 Faculty & Staff

**Convenience & Reliability**

Frequent customers who value convenience, product availability, and consistent store information.

:::

::: {.column width="33%"}

### 🎁 Alumni

**Emotional Connection**

High-value seasonal and gift shoppers who rely heavily on digital channels to stay connected.

:::

::::

:::: {.columns}

::: {.column width="50%"}

### 🏡 Local Community Members

**Local & Quality Focused**

Off-campus shoppers interested in local products but challenged by awareness, parking, navigation, and store information.

:::

::: {.column width="50%"}

### 🛒 Online-First Shoppers

**Digital Convenience**

Customers who expect clear product information, easy online ordering, secure checkout, and convenient pickup.

:::

::::

::: {.callout-important}
## Key Insight

**Local Community Members and Online-First Shoppers experience the greatest friction.**

The biggest omnichannel opportunity is to create a seamless customer journey from **Discovery → Product Research → Online Ordering → Store Pickup → Repeat Purchase**.
:::

## Target Customers
CPP Farm Store serves five key customer groups:
- ** Students ** - Convenient on-campus shoppers who value affordability, local products, and digital promotions.
- ** Faculty and staff** -Frequent customers who prioritize convenience, reliable product availability, and consistent store information.
- [Alumni：Emotionally connected customers who are especially valuable for seasonal and gift purchases but depend heavily on digital channels.]
- [Local community members：Off-campus shoppers interested in local products who face additional barriers related to awareness, campus navigation, parking, and store information.]
- [Online-first shoppers：Digitally driven customers who expect clear product information, online ordering, secure checkout, and convenient pickup.
]
Among these groups, Local Community Members and Online-First Shoppers experience the greatest friction, making them especially important when designing the omnichannel experience.


## Key Journey Insight

- [Main pain point：The largest gap is not product quality, but the disconnect between customer touchpoints. Customers may discover CPP Farm Store but struggle to find complete product information, confirm availability, understand online ordering, or confidently navigate the pickup process.]
- [Important customer touchpoints：Google Search, Google Maps, Shopify, product pages, Instagram, email, campus events, in-store signage, order confirmations, store pickup, employees, and product packaging all influence the customer experience.]
- [Biggest omnichannel opportunity：Connect these individual touchpoints into one continuous journey so customers can move easily from 

discovery → product research → online purchase → store pickup → post-purchase engagement → repeat purchase.]

## Customer Journey Opportunity

- [Online discovery：Improve visibility through Google Search, Google Merchant Center, social media, email, campus events, and in-store promotion. Customers should immediately understand that CPP Farm Store is open to the public and offers unique campus-grown products.
]
- [Product research：Use Shopify to provide clear product photos, descriptions, pricing, availability, seasonal collections, store hours, location, and parking information. Better information reduces uncertainty during the Consideration stage.]
- [Online ordering：Create a simple ecommerce experience with clear customization options, secure checkout, transparent pricing, and automatic order confirmation. This is especially important for custom gift boxes and Online-First Shoppers.]
- [Store pickup：Connect online purchasing to the physical store through scheduled pickup times, designated parking locations, clear directions, curbside options, and pickup notifications. This reduces one of the largest barriers for off-campus customers.]
- [Post-purchase engagement：Continue the relationship through thank-you emails, seasonal promotions, personalized recommendations, loyalty incentives, and upcoming product announcements. This turns a one-time transaction into an opportunity for repeat purchase and long-term loyalty.]

Key Takeaway: CPP Farm Store does not need to treat digital and physical channels as separate experiences. The strongest opportunity is to connect discovery, ecommerce, pickup, and customer retention into one seamless omnichannel journey.


# 4. Shopify / Ecommerce Recommendation

## Shopify Prototype

- [Purpose of Shopify prototype]
- [Main product categories]
- [Homepage strategy]
- [Navigation strategy]

## Customer Experience Improvements

- [Product pages]
- [Online ordering]
- [Gift Basket form]
- [Store pickup]
- [Mobile-friendly shopping]

## Prototype Evidence

[Insert Shopify screenshot here]


# 5. Product Feed / Retail Media Recommendation

### Product Feed Strategy

- **Selected products:** Fresh Fruit Boxes (Avocado/Mandarin, Satsuma, Deluxe, Navel), Build-Your-Own Boxes (Non Perishable, Fruit & Snacks, Farm Store, Anti Pasti, Pasta, Sweet), Seasonal Baskets (Christmas, Custom Christmas, Valentine's)
- **Product titles:** Keep the current recognizable names (e.g., "Deluxe Box," "Satsuma Gift Box") but standardize formatting across the feed
- **Descriptions:** Fresh Fruit Boxes note they're packed with CPP-grown fruit available at time of order; Build-Your-Own Boxes explain customers can shop in-store or call with a budget to have one built
- **Prices:** Not currently listed on the website — this is a gap to fix once the site can process orders
- **Availability:** Fresh Fruit Boxes are seasonal (depends on what's ripe); Build-Your-Own Boxes and Baskets are available year-round on request
- **Images:** Existing photos are usable but small and inconsistently cropped — recommend a refresh so all products share the same background and framing
- **Pickup information:** Gift Boxes ship anywhere in the US or can be picked up in-store; Gift Baskets are pickup-only

### Recommended Products for Promotion

- **Deluxe Box** — most visually appealing fresh-fruit option, strongest "grown here" story
- **Farm Store Gift Box** — flexible build-your-own option, appeals to a range of budgets
- **Custom Christmas Basket** — highest perceived value, ideal for the holiday gifting window

### Retail Media Opportunity

- **Google Merchant Center:** Focus on the shippable Gift Boxes, since Baskets are pickup-only and don't fit standard shipping-based retail feeds
- **Google Shopping:** Fresh Fruit Boxes are the most differentiated — no local competitor can claim "CPP-grown" produce
- **Improved online product discovery:** Current Wix site can't process orders — this feed only becomes actionable once the group's Shopify recommendation is live

# 6. Campaign Strategy

### Business Problem

- CPP Farm Store's current website runs on Wix, which cannot process online orders — customers must call or visit in person, no matter how much interest they have after browsing

### Campaign Objective

- Increase awareness and adoption of CPP Farm Store's online gift box ordering option.
- Drive customers from digital channels to Shopify and in-store pickup.

### Campaign Message

**Thoughtful Gifts, Grown Here**

### Target Audience

- Students
- Families
- Faculty and staff
- Campus organizations
- Local customers

### Channels

- Instagram
- TikTok
- Google Search
- Google Shopping
- Email
- Campus signage
- Shopify

### Campaign KPIs

- Website traffic
- Product page views
- Click-through rate
- Add-to-cart rate
- Conversion rate
- Purchases
- Revenue


# 7. Creative Examples

## Creative Objective

- [Insert creative objective]
- [Main message]
- [Tone and style]

## Creative Assets

[Insert creative asset screenshot]

## Channel Fit

- [Instagram]
- [TikTok]
- [Email]
- [Digital advertising]

## AI Use

- [Explain how AI was used]
- [Explain human review]
- [Explain how assets match CPP Farm Store branding]


# 8. Dashboard and Measurement Plan

## Analytics Dashboard

The CPP Farm Store Omnichannel Performance Dashboard was developed in Looker Studio to demonstrate how CPP Farm Store could monitor ecommerce, marketing, product, and omnichannel performance.

The dashboard uses **simulated data for educational purposes** because complete live CPP Farm Store ecommerce data were not available.

## Measurement Objectives

The dashboard is designed to:

- Measure website traffic and customer activity
- Track product engagement
- Compare marketing-channel performance
- Evaluate campaign effectiveness
- Monitor purchases and revenue
- Identify high-performing products
- Measure store pickup interest
- Support future data-driven decisions

## Key Performance Indicators

- **Total Users:** 980
- **Sessions:** 1,138
- **Product Views:** 796
- **Purchases:** 126
- **Revenue:** $2,644
- **Conversion Rate:** 11.07%
- **Average Order Value:** $20.98
- **Store Pickup Interest:** 65

## Dashboard Visualizations

The dashboard includes:

- Website Sessions Over Time
- Sessions by Marketing Channel
- Purchases by Campaign
- Product Performance
- Campaign Performance
- Conversion Rate
- Average Order Value
- Store Pickup Interest

## Dashboard

![CPP Farm Store Omnichannel Performance Dashboard](images/Final%20Looker%20Studio%20Dashboard.png){width="95%"}

## Key Dashboard Insights

- Multiple channels contribute to website traffic, supporting an omnichannel marketing approach.
- High website traffic does not automatically mean stronger campaign performance.
- Campaigns should be evaluated using purchases, revenue, and conversion rate in addition to sessions.
- Product-level performance can identify where customers show interest but do not complete a purchase.
- Store Pickup Interest helps connect digital activity with the physical CPP Farm Store.

## What Should Be Measured?

The dashboard should help CPP Farm Store determine whether the new digital Gift Basket ordering process is improving customer behavior.

Important measures include:

- Gift Basket product page views
- Online Gift Basket form starts
- Completed Gift Basket orders
- Add-to-cart activity
- Conversion rate
- Revenue
- Average Order Value
- Store Pickup Interest
- Traffic source

## Recommended Measurement Actions

CPP Farm Store should:

- Monitor traffic and conversion performance together.
- Invest more heavily in campaigns that generate purchases and revenue.
- Improve product pages when views do not translate into purchases.
- Make store pickup highly visible throughout the ecommerce experience.
- Regularly review dashboard data and adjust marketing decisions.
- Replace simulated data with live GA4 and Shopify data when available.

## Measurement Opportunity

A digital Gift Basket ordering process would provide CPP Farm Store with data that a paper form cannot easily provide.

With Shopify and GA4, the Farm Store could measure:

**Discovery → Product View → Order Intent → Completed Order → Store Pickup**

This would allow management to identify where customers leave the ordering process and continuously improve the experience.


# 9. Final Recommendations

## Recommendation 1: Highest Priority

**[Insert highest-priority recommendation]**

- What CPP Farm Store should do:
  - [Improve Shopify product pages, simplify checkout, automate order confirmations, and provide clear pickup, parking, and store information.]
- Why it matters:
  - [The greatest customer friction occurs between product consideration, online purchase, and physical pickup.
]
- Resources needed:
  - [Shopify updates • Product photos/content • Email automation • Pickup signage • Staff training
]
- Measurement:
  - [Conversion Rate • Cart Abandonment • Purchases • Store Pickup Interest • AOV]

## Recommendation 2: Medium-Term Opportunity

**[Insert recommendation]**

- What CPP Farm Store should do:
  - [Launch campaign across Google, email, campus events, in-store QR codes, and the Shopify gift box landing page.]
- Why it matters:
  - [Custom gift boxes combine CPP's local story with high-demand gifting occasions such as Graduation, Mother's Day, and holidays.]
- Resources needed:
  - [Google Ads • Email marketing • Campaign creative • Event displays • QR codes • Shopify landing page]
- Measurement:
  - [+25% Product Page Traffic • 3% Conversion Rate • +40% Gift Box Orders]

## Recommendation 3: Future Opportunity

**[Insert recommendation]**

- What CPP Farm Store should do:
  - [Introduce automated post-purchase emails, seasonal reminders, personalized recommendations, and loyalty rewards.]
- Why it matters:
  - [Turning satisfied first-time shoppers into repeat customers reduces dependence on continuous customer acquisition.]
- Resources needed:
  - [Email/CRM platform • Shopify integration • Loyalty tools • Customer segmentation]
- Measurement:
  - [Repeat Purchase Rate • Returning Customers • Email CTR • Customer Lifetime Value • 20% Repeat-Purchase Goal]


# 10. Closing

## From Paper Forms to a Connected Customer Journey

One of the clearest opportunities identified in this project is CPP Farm Store's Gift Basket ordering process.

Instead of requiring customers to visit the store and complete a paper order form, CPP Farm Store can allow customers to **discover, customize, and order Gift Baskets online through Shopify**.

The physical Farm Store would remain an important part of the experience through preparation, fulfillment, and customer pickup.

## Recommended Customer Journey

**Discover Online**

↓

**View Gift Basket Options**

↓

**Customize & Order Through Shopify**

↓

**Receive Order Confirmation**

↓

**Pick Up at CPP Farm Store**

↓

**Receive Follow-Up Marketing**

## Overall Value

This strategy connects every major part of our project:

- The **customer journey** identifies the friction.
- **Shopify** provides the digital ordering solution.
- The **product feed** improves online discovery.
- The **campaign** promotes the new ordering option.
- The **creative assets** communicate the offering.
- The **analytics dashboard** measures whether customers use it.

## Final Takeaway

CPP Farm Store does not need ecommerce to replace the physical Farm Store.

It can use ecommerce to eliminate unnecessary steps such as paper Gift Basket ordering while making the physical store more convenient and valuable.

**The goal is a simpler journey: order online, pick up at the Farm Store.**


# 11. Appendix

## Project Links

- **Final Report:** [Insert link]
- **GitHub Pages:** [Insert link]
- **GitHub Repository:** [Insert link]
- **Shopify Prototype:** [Insert link]
- **Looker Studio Dashboard:** [<https://datastudio.google.com/reporting/f8db8c82-30a9-4c24-a7f8-04d69d73dd7c>]
- **Google Drive Folder:** [Insert link]
- **Product Feed:** [Insert link]

## Additional Evidence

- [Shopify screenshots]
- [Gift Basket form screenshots]
- [Creative assets]
- [Product feed]
- [Dashboard]
- [Additional project materials]

# Thank You

## Questions?
